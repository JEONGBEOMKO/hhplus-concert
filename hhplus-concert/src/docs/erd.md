...
%% concert_schedules 테이블: 콘서트 일정 정보 관리
    concert_schedules {
        bigint id PK
...
%% 테이블 간 관계
    ...
    concerts ||--o{ concert_schedules : "has schedule"
    concert_schedules ||--o{ seats : "has seats"
    seats ||--o{ reservations : "used in reservation"
    reservations ||--o{ payments : "has payment"
    users ||--o{ queue : "has queue"
...
- **컬럼**:
    ...
    - `reserved_at`: 예약 시간
    - `reserved_expires_at`: 임시 예약의 만료 시간
    - `created_at`: 예약 생성 시간

...
public class ChargeBalanceUseCaseTest {
    ...
    public void testChargeBalance_Success() {
        ...
        // When
        long newBalance = chargeBalanceUseCase.chargeBalance(userId, amountToCharge);

        // Then
        assertEquals(1500L, newBalance, "잔액 충전 후 새로운 잔액은 1500이어야 합니다.");
    ...
    // 실패 테스트: 존재하지 않는 사용자로 인한 잔액 충전 실패
    @Test
    ...
    // 실패 테스트: 존재하지 않는 사용자로 인한 잔액 조회 실패
    @Test

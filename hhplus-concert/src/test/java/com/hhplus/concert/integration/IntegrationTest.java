...
import com.hhplus.concert.application.dto.response.*;
...
public class IntegrationTest {
    ...
    public void testQueueTokenCreationAndPolling() {
        // Token 생성 요청
        QueueRequest queueRequest = new QueueRequest(userId);
        QueueResponse queueResponse = new QueueResponse(userId, "generated-token", "WAITING", LocalDateTime.now(), LocalDate.now().atStartOfDay().plusMinutes(5), 1);

        // Assert queue 생성 검증
        assertNotNull(queueResponse.getToken(), "유저 대기열 토큰이 생성되어야 합니다.");
        assertEquals("WAITING", queueResponse.getStatus(), "대기열 초기 상태는 'WAITING'이어야 합니다.");

        // Queue polling 검증
        QueueResponse polledQueueResponse = queueResponse;
        assertEquals(queueResponse.getQueuePosition(), polledQueueResponse.getQueuePosition(), "대기열에서 조회한 순번이 일치해야 합니다.");
    }
    ...
    public void testAvailableDatesAndSeats() {
        // 예약 가능한 날짜 조회
        ConcertScheduleRequest dateRequest = new ConcertScheduleRequest(1L, "2024-10-25");
        ConcertScheduleResponse dateResponse = new ConcertScheduleResponse(1L, 1L, LocalDate.parse("2024-10-25"), null, null, 50, 50, "AVAILABLE");
        ...
        // 예약 가능한 좌석 조회
        SeatRequest seatRequest = new SeatRequest(1L);
        SeatResponse seatResponse = new SeatResponse(1L, 1L, 1000L, 1, "AVAILABLE");

        assertNotNull(seatResponse, "예약 가능한 좌석 응답은 null이 아니어야 합니다.");
        assertEquals("AVAILABLE", seatResponse.getSeatStatus(), "예약 가능한 좌석 상태는 'AVAILABLE'이어야 합니다.");
    }

    @Test
    @DisplayName("좌석 예약 요청 테스트")
    public void testReserveSeat() {
        ReservationRequest reservationRequest = new ReservationRequest(userId, 1L, 1000L, 1);
        ReservationResponse reservationResponse = new ReservationResponse(1L, userId, 1L, 1000L, 1, "TEMPORARY", LocalDateTime.now(), LocalDateTime.now().plusMinutes(5));

        assertEquals("TEMPORARY", reservationResponse.getStatus(), "좌석 예약 상태는 'TEMPORARY'이어야 합니다.");
        assertNotNull(reservationResponse.getReservedExpiresAt(), "예약 만료 시간은 null이 아니어야 합니다.");
        assertTrue(reservationResponse.getReservedExpiresAt().isAfter(LocalDateTime.now()), "예약 만료 시간이 현재 시간 이후여야 합니다.");
    }
    ...
    public void testChargeAndGetBalance() {
        ...
        // Then
        assertEquals(15000L, chargeResponse.getNewBalance(), "충전 후 잔액은 15000이어야 합니다.");

        // 잔액 조회
        long balance = chargeResponse.getNewBalance();
